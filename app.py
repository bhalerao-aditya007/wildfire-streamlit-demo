...
def create_confidence_chart(raw_prob):
    """Create an elegant bar chart visualization"""
    fig, ax = plt.subplots(figsize=(12, 6), facecolor='white')
    
    # Data
    categories = ['No Wildfire', 'Wildfire']
    probabilities = [1 - raw_prob, raw_prob]
    colors = ['#2ecc71', '#e74c3c']
    
    # Create horizontal bars with gradient effect
    bars = ax.barh(categories, probabilities, height=0.6, color=colors, alpha=0.85, edgecolor='white', linewidth=2)
    
    # Add subtle gradient overlay
    for i, (bar, prob, color) in enumerate(zip(bars, probabilities, colors)):
        # Add inner glow effect
        ax.barh(categories[i], prob, height=0.4, color=color, alpha=0.3)
    ...
    # Styling
    ax.set_xlim(0, 1)
    ax.set_xlabel('Probability', fontsize=14, fontweight='600', color='#2c3e50')
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.spines['left'].set_linewidth(2)
    ax.spines['bottom'].set_linewidth(2)
    ax.tick_params(axis='both', which='major', labelsize=12, colors='#2c3e50', width=2)
    ax.set_yticks(range(len(categories)))
    ax.set_yticklabels(categories, fontsize=14, fontweight='600', color='#2c3e50')
    ...
    ax.axvline(0.5, color='#7f8c8d', linestyle='--', linewidth=2, alpha=0.5, label='Decision Threshold')
    ...
    # Add text box with risk assessment
    confidence = max(raw_prob, 1 - raw_prob)
    textstr = f'Risk Level: {risk_text}\nModel Confidence: {confidence*100:.1f}%'
    props = dict(boxstyle='round,pad=0.8', facecolor=risk_color, alpha=0.15, edgecolor=risk_color, linewidth=2.5)
    ax.text(0.98, 0.97, textstr, transform=ax.transAxes, fontsize=13,
            verticalalignment='top', horizontalalignment='right', bbox=props, 
            fontweight='600', color=risk_color)
    
    # Title
    ax.set_title('Wildfire Detection Analysis', fontsize=18, fontweight='bold', 
                color='#2c3e50', pad=20)
    
    # Legend
    ax.legend(loc='lower right', frameon=True, fontsize=11, fancybox=True, shadow=True)
